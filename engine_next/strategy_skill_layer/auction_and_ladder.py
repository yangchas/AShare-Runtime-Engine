...
def _conservative_watch_action_for_selection(stock_selection: StockSelectionContext | None) -> str:
    if stock_selection is None:
        return "observe_only"
    if stock_selection.is_true_leader:
        return "leader_watch"
    ...
    *,
    ...
    if theme_selection is not None and not theme_selection.tradable:
        ...
        if repair_probe_candidate:
            ...
            fallback_watch = _conservative_watch_action_for_selection(stock_selection)
            confidence = 28 if fallback_watch != "observe_only" else 20
            setup_id = "theme_not_tradable_guard"
            action = fallback_watch
            reasons.append(
                "theme is not tradable yet, so only conservative leader/front-row watch is allowed"
                if fallback_watch != "observe_only"
                else "theme is not tradable, so stock-level strength is not enough"
            )
        ...
        if stock_selection is not None and stock_selection.is_true_leader:
            ...
            fallback_watch = _conservative_watch_action_for_selection(stock_selection)
            confidence = 26 if fallback_watch != "observe_only" else 18
            setup_id = "open_confirm_falsified_guard"
            action = fallback_watch
            reasons.append(
                "open confirmation falsified the theme, so only conservative leader/front-row watch survives"
                if fallback_watch != "observe_only"
                else "open confirmation falsified the theme, so entries are blocked"
            )
    elif (
        ...
        fallback_watch = _conservative_watch_action_for_selection(stock_selection)
        confidence = 30 if fallback_watch != "observe_only" else 22
        setup_id = "theme_fakeout_guard"
        action = fallback_watch
        reasons.append(
            "theme fakeout risk blocks active participation, keep only conservative leader/front-row watch"
            if fallback_watch != "observe_only"
            else "theme fakeout risk blocks non-leader participation"
        )
    ...
    if risk_reward < 2.0 and action not in ("hold_only", "observe_only", "avoid_after_failed_promotion", "do_not_chase"):
        downgraded_watch = _conservative_watch_action_for_selection(stock_selection)
        action = downgraded_watch if downgraded_watch != "observe_only" else "observe_only"
